...
function AppointmentTable() {
  ...
  const url = "http://localhost:3001/api/appointment/get-all-appointments";
  ...
  function callApiDeleteappointment(id) {
    const url = "http://localhost:3001/api/appointment/update-appointment";
    const data = {
      appointment_id: id,
      status: "canceled",
    };
  ...
  const areSureDelete = (chosoe) => {
    if (chosoe) {
      const temp = appointments.filter(
        (appointment) => appointment["id"] == idappointment.current
      );
  ...
  return (
    <div className={cx("wrapper-table")}>
      ...
      {dialogE.isLoadingE && (
        <Dialog onDialog={areSureExamine} message={dialogE.messageE} />
      )}
    </div>
  );
}

...
function MakeAppointment() {
  ...
  function CallApiMakeAppointment() {
    const url = "http://localhost:3001/api/appointment/make-appointment";
    const data = {
      patient_id: userID,
      day: dateChoose,
      time: timeChoose,
  ...
  const handleSubmit = () => {
    ...
    chooseHour.setHours(Number(hour), Number(minute), 0); // timeChoose
  ...
  return (
    <div className={cx("wrapper-ma")}>
      <div className={cx("inner-ma")}>
        <div className={cx("title-ma")}>Đặt Lịch Khám</div>
            ...
            <form className={cx("form_content")}>
              ...
              {isSubmit && !dateChoose && (
                <label className="warning">
                  {" "}
                  <RiErrorWarningFill /> Chưa chọn ngày đặt lịch
                </label>
              )}

              <label className={cx("title-input")}>
                Thời Gian 8:00-AM to 19:30-PM
              </label>
              ...
              {isSubmit && !timeChoose && (
                <label className="warning">
                  {" "}
                  <RiErrorWarningFill /> Chưa chọn giờ đặt lịch
                </label>
              )}
              {isSubmit && isErrorTime && (
                <label className="warning">
                  {" "}
                  <RiErrorWarningFill /> Giờ đặt lịch từ 8:00-AM to 19:30-PM
                </label>
              )}
              ...
              {isSubmit && !description && (
                <label className="warning">
                  {" "}
                  <RiErrorWarningFill /> Chưa điền mô tả đặt lịch
                </label>
              )}
            </form>
            <button

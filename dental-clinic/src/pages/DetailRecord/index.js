...
function DetailRecord() {
  ...
  const url = `http://localhost:3001/api/authentication/get-patient-detail`;
  useEffect(() => {
  ...
  const urlRec = `http://localhost:3001/api/medical_record`;
  useEffect(() => {
    if (param.recID) {
  ...
  const urldoc = "http://localhost:3001/api/authentication/get-profile";
  const data = {
  ...
  function callApiAddRecord() {
    const url = "http://localhost:3001/api/add_medical_record";
    const data = {
      d_id: dID,
      p_id: param.userID,
      rec_dease: dease,
  ...
  function callApiUpateRecord() {
    const url = "http://localhost:3001/api/record/update";
    const data = {
  ...
  return (
    <div className={cx("wrapper-detail-record")}>
      <div className={cx("inner-detail-record")}>
        <div className={cx("title-detail-record")}>Hồ sơ bệnh án nha khoa</div>
        <div className={cx("container-detail-record")}>
          <form className={cx("form-record")}>
            ...
            <label className={cx("info-readonly")}>
              {userProfile["p_name"]}
            </label>
            ...
            />
            <label className={cx("title-input-record")}>
              Phương pháp đề xuất
            </label>
            <input
              className={cx("input-record")}
          ...
          {isInfo && (
            <button
              ...
              className={cx("btn-submit-record")}
            >
              Lưu
            </button>
          )}
      ...
      {dialog.isLoading && (
        <Dialog onDialog={areSureDelete} message={dialog.message} />
      )}
    </div>
  );
}
